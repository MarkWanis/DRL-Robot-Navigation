...
class GazeboEnv(Node):
    ...
    def step(self, action):
        global velodyne_data, last_odom
        target = False
        ...
        vel_cmd = Twist()
        vel_cmd.linear.x = linear_damp * float(action[0])
        vel_cmd.angular.z = angular_damp * float(action[1])
        ...
        if last_odom is None:
            self.get_logger().warn("No odom received; using zeros")
            self.odom_x = 0.0
            self.odom_y = 0.0
            angle = 0.0
...
class Velodyne_subscriber(Node):
    ...
    def scan_callback(self, msg):
        ...
        for chunk_index in range(20):
            chunk_start = int(len(msg.ranges) * chunk_index / 20)
            chunk_end = int(len(msg.ranges) * (chunk_index+1) / 20)
            #velodyne_data[chunk_index] = min(msg.ranges[chunk_start:chunk_end])
            for scan_index, scan_value in enumerate(msg.ranges[chunk_start:chunk_end]):
                if not np.isnan(scan_value) and scan_value < velodyne_data[chunk_index]: 
                    velodyne_data[chunk_index] = scan_value
